...
### [V1.3] 作业分类与交互式诊断
*   **作业细分标签**: 引入了“重大成绩 (Major Grade)”、“测验 (Quiz)”、“普通作业”及“日常练习”分类，支持 AI 自动识别分类或手动标注，优化任务优先级管理。
*   **交互式掌握度分析**: 扫描报告中的知识点点击即可展开，实时展示 mastery 级别详情及针对性的 AI 诊断建议，让评估更加直观。

### [V1.4] 学习追踪与智能过滤
*   **可视化进度管理**: Learning Hub 新增动态进度环与任务核查机制，基于任务完成状态实时计算学习计划的总体进度百分比。
*   **多维过滤器**: 仪表盘作业列表现已支持按科目、作业类别及截止日期进行综合筛选，即便面对海量任务也能快速定位目标。
