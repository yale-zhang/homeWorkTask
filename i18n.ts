
import React, { createContext, useContext, useState, useEffect, ReactNode } from 'react';

export type Language = 'en' | 'zh';

interface Translations {
  [key: string]: {
    [key in Language]: string;
  };
}

const translations: Translations = {
  // Navigation
  nav_dashboard: { en: 'Dashboard', zh: '仪表盘' },
  nav_inbox: { en: 'Assignments', zh: '作业采集' },
  nav_scanner: { en: 'AI Scanner', zh: 'AI 批改' },
  nav_learning: { en: 'Learning Hub', zh: '学习中心' },
  nav_reports: { en: 'Reports', zh: '学情报告' },
  nav_exams: { en: 'Exam Center', zh: '考试中心' },
  workspace: { en: 'Workspace', zh: '工作空间' },
  settings: { en: 'Settings', zh: '设置' },
  
  // Academic Identity & Events
  academic_identity: { en: 'Student Profile', zh: '学生档案' },
  term_first: { en: 'First Semester', zh: '上学期' },
  term_second: { en: 'Second Semester', zh: '下学期' },
  week_prefix: { en: 'Week ', zh: '第 ' },
  week_suffix: { en: '', zh: ' 周' },
  semester_progress: { en: 'Semester Progress', zh: '学期进度' },
  current_grade: { en: 'Grade', zh: '年级' },
  school: { en: 'School', zh: '学校' },
  edit_profile: { en: 'Edit Profile', zh: '编辑资料' },
  save_changes: { en: 'Save Changes', zh: '保存修改' },
  nickname: { en: 'Nickname', zh: '昵称' },
  avatar_url: { en: 'Avatar URL', zh: '头像地址' },
  upcoming_milestones: { en: 'Academic Milestones', zh: '学术里程碑' },
  prep_advice_title: { en: 'AI Prep Strategy', zh: 'AI 备考建议' },
  milestone_school: { en: 'School Level', zh: '学校级' },
  milestone_class: { en: 'Class Level', zh: '班级级' },
  milestone_manage: { en: 'School & Milestones', zh: '学校与节点' },
  milestone_nodes: { en: 'Managed Schools', zh: '学校管理' },
  add_node: { en: 'Add Event', zh: '添加事件' },
  add_school: { en: 'Add School', zh: '添加学校' },
  new_event: { en: 'New Event', zh: '新事件' },
  new_school: { en: 'New School', zh: '新学校' },
  no_nodes_yet: { en: 'No events defined for this school.', zh: '该学校尚未定义学术事件。' },

  // Exam Center
  exam_center_title: { en: 'Exam Prep Center', zh: '考试备考中心' },
  exam_center_desc: { en: 'Data-driven strategies derived from your daily performance.', zh: '基于日常表现的数据驱动型备考策略。' },
  previous_stage_perf: { en: 'Previous Stage Performance', zh: '前一阶段表现分析' },
  ai_strategy: { en: 'AI Strategic Advice', zh: 'AI 备考方针' },
  no_prev_data: { en: 'No sufficient data from previous stage yet.', zh: '暂无足够的前一阶段数据。' },
  gen_advice: { en: 'Generate Strategy', zh: '生成策略' },

  // Event Types
  'Daily Homework': { en: 'Daily Homework', zh: '课后作业' },
  'Weekly Quiz': { en: 'Weekly Quiz', zh: '单周测试' },
  'Monthly Test': { en: 'Monthly Test', zh: '月度测试' },
  'Midterm Exam': { en: 'Midterm Exam', zh: '期中考试' },
  'Final Exam': { en: 'Final Exam', zh: '期末考试' },

  // Auth
  login_welcome: { en: 'Welcome to IntelliTask', zh: '欢迎使用 IntelliTask' },
  login_desc: { en: 'Sign in to sync your learning data across devices.', zh: '登录以同步您的多端学习数据。' },
  login_email: { en: 'Email Address', zh: '邮箱地址' },
  login_password: { en: 'Password', zh: '密码' },
  login_remember_me: { en: 'Remember Me', zh: '记住密码' },
  login_signin_btn: { en: 'Sign In', zh: '登录' },
  login_signup_btn: { en: 'Sign Up', zh: '注册账号' },
  login_no_account: { en: "Don't have an account?", zh: '还没有账号？' },
  login_has_account: { en: 'Already have an account?', zh: '已有账号？' },
  login_other_methods: { en: 'Or continue with', zh: '或者通过以下方式登录' },
  wechat_login: { en: 'WeChat', zh: '微信登录' },
  github_login: { en: 'GitHub', zh: 'GitHub 登录' },
  switch_account: { en: 'Switch Account', zh: '切换账号' },
  logout: { en: 'Logout', zh: '退出登录' },
  auth_loading: { en: 'Processing...', zh: '正在处理...' },
  account_center: { en: 'Account Center', zh: '账号中心' },
  invalid_creds: { en: 'Invalid email or password', zh: '邮箱或密码错误' },
  email_exists: { en: 'Email already registered', zh: '该邮箱已被注册' },
  signup_success: { en: 'Account created successfully', zh: '账号注册成功' },
  login_success: { en: 'Welcome back!', zh: '欢迎回来！' },

  // Settings
  save_settings: { en: 'Save Settings', zh: '保存设置' },
  settings_saved: { en: 'Settings saved successfully', zh: '设置保存成功' },
  reset_settings: { en: 'Reset to Default', zh: '恢复默认' },
  
  // Dashboard
  welcome: { en: 'Welcome back, {name}!', zh: '欢迎回来，{name}！' },
  pending_desc: { en: 'You have {count} assignments to complete this week.', zh: '你本周有 {count} 项作业待完成。' },
  stat_pending: { en: 'Pending Tasks', zh: '待处理任务' },
  stat_completed: { en: 'Completed', zh: '已完成' },
  stat_avg_score: { en: 'Avg Score', zh: '平均分' },
  stat_gaps: { en: 'Knowledge Gaps', zh: '知识薄弱点' },
  recent_assignments: { en: 'Recent Assignments', zh: '最近作业' },
  all_subjects: { en: 'All Subjects', zh: '全部科目' },
  all_categories: { en: 'All Categories', zh: '全部类型' },
  clear_filters: { en: 'Clear', zh: '清除' },
  no_tasks: { en: 'No assignments match your filters.', zh: '没有匹配筛选条件的作业。' },
  
  // Inbox
  inbox_title: { en: 'Assignment Inbox', zh: '作业采集箱' },
  inbox_desc: { en: 'Monitoring school group messages for upcoming homework.', zh: '实时监控学校群聊，提取最新作业。' },
  add_manual: { en: 'Add Manual', zh: '手动录入' },
  active_monitoring: { en: 'Active Monitoring', zh: '监控运行中' },
  ai_extract: { en: 'AI Extract', zh: 'AI 提取' },
  create_new: { en: 'Create New Assignment', zh: '创建新作业' },
  subject: { en: 'Subject', zh: '科目' },
  category: { en: 'Category', zh: '类型' },
  due_date: { en: 'Due Date', zh: '截止日期' },
  details: { en: 'Details', zh: '详情描述' },
  cancel: { en: 'Cancel', zh: '取消' },
  add_btn: { en: 'Add Assignment', zh: '添加作业' },
  upload_photo: { en: 'Upload Photo', zh: '上传照片' },
  
  // Scanner
  scanner_title: { en: 'AI Homework Scanner', zh: 'AI 作业扫描仪' },
  scanner_desc: { en: 'Submit your completed work for instant grading and gap analysis.', zh: '上传已完成的作业，获取即时批改与薄弱点分析。' },
  select_task: { en: 'Select an Assignment to Grade', zh: '选择要批改的作业' },
  grading_analysis: { en: 'Grading Analysis', zh: '批改分析报告' },
  final_score: { en: 'Final Score', zh: '最终得分' },
  strengths: { en: 'Key Strengths', zh: '核心优势' },
  weaknesses: { en: 'Areas to Improve', zh: '待改进领域' },
  transcript: { en: 'Handwriting Transcript', zh: '手写内容转录' },
  breakdown: { en: 'Knowledge Point Breakdown', zh: '知识点掌握分布' },
  view_plan: { en: 'View Learning Plan', zh: '查看学习计划' },
  batch_grade: { en: 'Smart Batch Grade', zh: '智能批量批改' },
  processing_tasks: { en: 'Processing...', zh: '正在处理...' },
  grading_done: { en: 'Grading complete. Starting learning plan generation...', zh: '批改完成。正在生成学习计划...' },
  
  // Learning Hub
  roadmap: { en: 'Personalized AI Roadmap', zh: '个性化 AI 学习路径' },
  plan_title: { en: 'Adaptive Learning Plan', zh: '自适应学习计划' },
  focus_areas: { en: 'Focus Areas', zh: '重点关注' },
  plan_progress: { en: 'Plan Progress', zh: '计划进度' },
  notif_plan_ready: { en: 'Success! Your custom learning plan for {focus} is ready.', zh: '成功！为您定制的针对 {focus} 的学习计划已就绪。' },
  notif_title: { en: 'Notifications', zh: '通知中心' },
  notif_empty: { en: 'No new notifications', zh: '暂无新通知' },
  generating_plan: { en: 'AI is generating your learning plan...', zh: 'AI 正在为您生成学习计划...' },
  
  // Reports
  reports_title: { en: 'Weekly Performance', zh: '周度学情报告' },
  reports_desc: { en: 'Aggregated data from your AI-graded assignments', zh: '基于 AI 批改结果的学情汇总' },
  export_pdf: { en: 'Export PDF', zh: '导出 PDF' },
  last_7_days: { en: 'Last 7 Days', zh: '最近 7 天' },
  score_completion_trend: { en: 'Score vs. Completion Trend', zh: '得分与完成率趋势' },
  avg_score_label: { en: 'Avg Score', zh: '平均分' },
  completion_label: { en: 'Completion', zh: '完成率' },
  subject_proficiency_map: { en: 'Subject Proficiency Map', zh: '科目能力图谱' },
  student_proficiency: { en: 'Student Proficiency', zh: '学生能力值' },
  critical_gaps: { en: 'Critical Knowledge Gaps', zh: '核心知识漏洞' },
  critical_gaps_desc: { en: 'Specific topics requiring immediate attention (< 70% Mastery)', zh: '需要立即关注的特定主题（掌握度 < 70%）' },
  mastery_level: { en: 'Mastery Level', zh: '掌握程度' },
  diagnostic_level: { en: 'Diagnostic Level: High', zh: '诊断等级：高' },
  no_critical_gaps: { en: 'No Critical Gaps Detected', zh: '未发现核心漏洞' },
  no_critical_gaps_desc: { en: 'Your mastery levels are looking solid! Keep scanning assignments to maintain this baseline.', zh: '您的掌握程度非常扎实！请继续保持目前的扫描频率。' },
  strategic_recommendation: { en: 'Strategic Recommendation', zh: '学习策略建议' },
  time_spent: { en: 'Time Spent', zh: '投入时长' },
  mistake_rate: { en: 'Mistake Rate', zh: '错题率' },
  peer_rank: { en: 'Peer Rank', zh: '同伴排名' },
  vs_last_week: { en: 'vs last week', zh: '对比上周' },
  steady: { en: 'Steady', zh: '稳定' },
  recommendation_gap: { en: 'Your data indicates a strong struggle with "{gap}". We recommend spending 20 minutes daily reviewing this specific concept before moving to higher-level topics.', zh: '数据表明您在“{gap}”上遇到了较大的困难。建议每天花 20 分钟复习该概念，然后再进行更高阶的学习。' },
  recommendation_solid: { en: 'Based on your {hours} hours of focus, your efficiency in {subject} has peaked. We recommend maintaining your current pace.', zh: '基于您 {hours} 小时的专注投入，您在 {subject} 上的效率已达顶峰。建议保持目前的学习节奏。' },

  Mathematics: { en: 'Mathematics', zh: '数学' },
  Science: { en: 'Science', zh: '科学' },
  English: { en: 'English', zh: '英语' },
  History: { en: 'History', zh: '历史' },
  Chinese: { en: 'Chinese', zh: '语文' },
  'Major Grade': { en: 'Major Grade', zh: '重要考试' },
  Quiz: { en: 'Quiz', zh: '小测验' },
  Homework: { en: 'Homework', zh: '普通作业' },
  'Daily Practice': { en: 'Daily Practice', zh: '日常练习' },
  Mon: { en: 'Mon', zh: '周一' },
  Tue: { en: 'Tue', zh: '周二' },
  Wed: { en: 'Wed', zh: '周三' },
  Thu: { en: 'Thu', zh: '周四' },
  Fri: { en: 'Fri', zh: '周五' },
  Sat: { en: 'Sat', zh: '周六' },
  Sun: { en: 'Sun', zh: '周日' }
};

interface LanguageContextType {
  language: Language;
  setLanguage: (lang: Language) => void;
  t: (key: string, params?: Record<string, string | number>) => string;
}

const LanguageContext = createContext<LanguageContextType | undefined>(undefined);

export const LanguageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [language, setLanguage] = useState<Language>(() => {
    return (localStorage.getItem('itask_lang') as Language) || 'zh';
  });

  useEffect(() => {
    localStorage.setItem('itask_lang', language);
    document.documentElement.lang = language;
  }, [language]);

  const t = (key: string, params?: Record<string, string | number>) => {
    let text = translations[key]?.[language] || key;
    if (params) {
      Object.entries(params).forEach(([k, v]) => {
        text = text.replace(`{${k}}`, v.toString());
      });
    }
    return text;
  };

  return React.createElement(
    LanguageContext.Provider,
    { value: { language, setLanguage, t } },
    children
  );
};

export const useTranslation = () => {
  const context = useContext(LanguageContext);
  if (!context) throw new Error('useTranslation must be used within a LanguageProvider');
  return context;
};
